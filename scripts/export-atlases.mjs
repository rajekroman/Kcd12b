...
const writeExports = async (exports, root = '.') => {
  for (const item of exports) {
    const path = resolve(root, item.entry.targetPath);
...
const artifactDir = artifactDirArg?.slice('--artifact-dir='.length);
const exports = await verifyDeterminism();

if (argSet.has('--check')) {
  await checkCommittedExports(exports);
} else if (artifactDir) {
  await writeExports(exports, artifactDir);
  await writeReport(exports, artifactDir);
} else if (!argSet.has('--verify-only')) {
