...
export class AdaptiveAudioController {
  ...
  constructor() {
    ...
    document.body.dataset.audioStatus = 'locked';
    this.renderButton();
  }
  ...
  async destroy(): Promise<void> {
    ...
    if (this.graph) {
      this.stopNode(this.graph.ambienceSource);
      this.stopNode(this.graph.droneLow);
      this.stopNode(this.graph.droneHigh);
      if (this.graph.context.state !== 'closed') await this.graph.context.close();
    }
  ...
  private async unlock(): Promise<void> {
    ...
    document.body.dataset.audioStatus = 'ready';
    this.refreshState();
    this.renderButton();
  }
  ...
  private refreshState(): void {
    const nextState = getMusicState({
      ...
      muted: this.muted || !this.unlocked || this.failed
    });

    const moodChanged = nextState.mood !== this.currentState.mood;
    this.currentState = nextState;
    document.body.dataset.musicMood = nextState.mood;
    document.body.dataset.musicMuted = String(this.muted || !this.unlocked || this.failed);
  ...
  private applyState(state: MusicState, moodChanged: boolean): void {
    ...
    graph.droneLow.frequency.setTargetAtTime(state.motif.rootFrequency * 0.5, now, 0.35);
    graph.droneHigh.frequency.setTargetAtTime(state.motif.rootFrequency * 0.75, now, 0.35);
  }
  ...
  private schedulePercussion(graph: AudioGraph, time: number, duration: number): void {
    ...
    filter.frequency.setValueAtTime(this.currentState.mood === 'alerted' ? 165 : 115, time);
  ...
  private createGraph(): AudioGraph {
    if (typeof window.AudioContext !== 'function') throw new Error('WebAudio is unavailable.');
  ...
  private rampGain(parameter: AudioParam, target: number, now: number, duration: number): void {
  ...
  private handleAudioFailure(): void {
    this.failed = true;
    this.muted = true;
    document.body.dataset.audioStatus = 'unavailable';
    document.body.dataset.musicMuted = 'true';
    document.body.dataset.musicMood = 'silent';
    this.button.disabled = true;
    this.button.textContent = 'Hudba nedostupná';
    this.button.title = 'WebAudio se nepodařilo spustit.';
  }
  ...
  private renderButton(): void {
    ...
    this.button.title = active ? 'Ztlumit hudbu (M)' : 'Zapnout adaptivní hudbu (M)';
  }

  private stopNode(node: AudioBufferSourceNode | OscillatorNode): void {
    try {
      node.stop();
    } catch {
      // Node may already have stopped during page teardown.
    }
  }
