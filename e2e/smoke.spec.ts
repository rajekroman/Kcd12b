...
interface StoredBrowserSave {
  ...
  world: { dayClock: number; huntedAnimals?: string[] };
  economy: {
    inventory: {
      groschen: number;
...
test('spotřební předmět obnoví zdraví a okamžitě vyvolá save verze 5', async ({ page }) => {
  ...
  expect(stored?.world.huntedAnimals).toEqual([]);
});

test('neutrální nákup a vybavení nové zbraně se uloží do verze 5', async ({ page }) => {
  await installSave(page, 'chronicles-of-bohemia.save.v2', {
    version: 2,
...
test('legacy save verze 1 se migruje do IndexedDB verze 5', async ({ page }) => {
